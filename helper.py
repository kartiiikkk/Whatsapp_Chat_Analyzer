...
def create_wordcloud(selected_user,df):
    ...
    wc=WordCloud(width=400, height=300,min_font_size=10,colormap="RdYlBu", background_color='white')
...
def most_common_words(selected_user,df):
    ...
    for message in temp['message']:
        for word in message.lower().split():
            if word not in stop_words:
                words.append(word)
    b_new=pd.DataFrame(Counter(words).most_common(20))
    return b_new
...
def emoji_count(selected_user,df):
    ...
    emoji_counts=pd.DataFrame(Counter(emojis).most_common(len(Counter(emojis))))
    return emoji_counts
